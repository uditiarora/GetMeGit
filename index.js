...
const app = express();
async function gitBot() {

    var user = new GhPolyglot('uditiarora' + "/git-stats");
    ...
    bot.onText(/\/start/, (msg) => {
        ...
        username_dict[chatId] = 'uditiarora';
        ...
        const username = username_dict[chatId];
    ...
    bot.onText(/\/user (.+)/, (msg, match) => {
        const chatId = msg.chat.id;
        username_dict[chatId] = match[1] === '' ? 'uditiarora' : match[1];
        const username = username_dict[chatId];
    ...
    bot.onText(/\/joining_date/, (msg) => {
        const chatId = msg.chat.id;
        if(user_data != null){
            bot.sendMessage(chatId, `User's joining date is ${user_data.created_at.substring(0,10)}.`);
        }
        else{
            bot.sendMessage(chatId,"Couldn't find this user. Try some other user");
        }    
